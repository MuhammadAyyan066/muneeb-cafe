...
document.addEventListener('DOMContentLoaded', () => {
  loadOrders();
  setInterval(loadOrders, 6000); // 6 sec auto-polling
});
...
function shareToWhatsApp(orderStr) {
  ...
  const message =
`*Muneeb Fast Food - Order Details*
------------------------------------
...
function renderOrders() {
  ...
  filtered.forEach(order => {
    ...
    const addressVal = order.address || order.deliveryAddress || 'No Delivery Address Specified';
    ...
    card.innerHTML = `
      ...
      <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 border-t border-neutral-700/60">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button onclick="shareToWhatsApp('${encodedOrder}')" class="bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition ml-auto sm:ml-2 shadow-sm">
