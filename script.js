...
const fallbackMenuItems = [
  ...
  { name: "Muneeb Special Platter", category: "Deals", price: 3000, time: "25 min", rating: "4.9 (78)", desc: "10 Nuggets, 10 Hot wings, 10 Grill wings, 1 Large Pizza, 1 Drink 1.5 Ltr", tag: "Platter", image: "images/deals.jpg" },
...
try {
  ...
  if (localCache) {
    cachedInitialItems = JSON.parse(localCache);
  }
} catch (e) {
  console.warn("Local cache read error", e);
}

let activeMenuItems = Array.isArray(cachedInitialItems) && cachedInitialItems.length > 0 
  ? cachedInitialItems 
  : [...fallbackMenuItems];

// Fresh cart on reload
